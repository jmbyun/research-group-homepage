...
class hdsp:
    ...
    def runVariationalEM(self, max_iter, corpus, isHDP=False):
        ...
        for iter in xrange(max_iter):
            ...
            if self.is_verbose and corpus.heldout_ids != None:
                ...
                print 'no init %d iter, %d topics, %.2f time, %.2f lower_bound %.3f heldout_ll' % (iter, self.K, time.clock()-curr, lb, perp)
            if self.is_verbose and corpus.heldout_ids == None:
                print 'no init %d iter, %d topics, %.2f time, %.2f lower_bound' % (iter, self.K, time.clock()-curr, lb)    
    ...
    def update_Z(self, corpus, iter):
        ...
        # expectation of p(Z)            
        lb += np.sum(-bp * ln_E_w + (bp-1)*(psi(corpus.A)-np.log(corpus.B)) - E_inv_w*(corpus.A/corpus.B) - gammaln(bp))
    ...
    def update_W(self, corpus):
        ...
        lb += np.sum(corpus.w_a_prior * np.log(corpus.w_b_prior) - gammaln(corpus.w_a_prior) - (corpus.w_a_prior + 1)*(np.log(corpus.w_B)-psi(corpus.w_A)) - corpus.w_b_prior * (corpus.w_A/corpus.w_B))
        lb -= np.sum(corpus.w_A * np.log(corpus.w_B) - gammaln(corpus.w_A) - (corpus.w_A+1)*(np.log(corpus.w_B)-psi(corpus.w_A)) - corpus.w_B * (corpus.w_A/corpus.w_B))
        if self.is_verbose:
    ...
    def update_V(self, corpus):
        ...
        sum_r_w = np.zeros(self.K)
        E_w_inv_doc_prod = np.zeros([corpus.M, self.K])
        sum_E_w_inv = np.zeros(self.K)
